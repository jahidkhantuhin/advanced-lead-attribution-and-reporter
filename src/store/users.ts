...
export const getters = {
  ...
  authenticatedUserRole(state: any, getters: any, rootState: any) {
    return rootState.auth && rootState.auth.user
      ? rootState.auth.user.data.role
      : null
  },
  authenticatedUserScope(state: any, getters: any, rootState: any) {
    return rootState.auth && rootState.auth.user && rootState.auth.user.data
      ? rootState.auth.user.data.scope
      : null
  },
  isAllowed: (state: any, getters: any, rootState: any) => {
    return (name: String) => {
      const scope = rootState.auth?.user?.data?.scope
      if (!name) {
        return true
      }
